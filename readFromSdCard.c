...
uint8_t redEmpty = false;
uint8_t blueEmpty = false;
uint8_t pullAudio = false;

uint16_t sampleRate = 1600;

typedef enum  {MIC_IDLE, MIC_ACQUIRE, SD_ACQUIRE} myTMR0states_t;
...
void main(void) {
    // moved sin into main per rubric
        ...
        if (EUSART1_DataReady) { // wait for incoming data on USART
            ...
            switch (cmd) { // and do what it tells you to do
                ...
                case '?':
                    ...
                    printf("sdCardAddress: %04x : %04x \r\n", sdCardAddress >> 16, sdCardAddress & 0X0000FFFF);
                ...
                case 'P':
                    printf("Playing Audio. Hit any key to exit. \r\n");
                    sdCardAddress = 0;
                    redEmpty = false;
                    blueEmpty = false;
                    ...
                    pullAudio = true;
                    while(!EUSART1_DataReady){
                        while(blueEmpty == false);
                        // fill blue buffer while ISR is reading from red buffer
                        SDCARD_ReadBlock(sdCardAddress, blueBuffer);
                        sdCardAddress =  sdCardAddress + BLOCK_SIZE;
                        // allow the ISR to pull from blue
                        blueEmpty = false;
                        
                        while(redEmpty == false);
                        // fill red buffer while ISR is reading from blue buffer
                        SDCARD_ReadBlock(sdCardAddress, redBuffer);
                        sdCardAddress =  sdCardAddress + BLOCK_SIZE;
                        // allow the ISR to pull from red
                        redEmpty = false;
                    }
                    pullAudio = false;
                    uint8_t tempNew = EUSART1_Read();
...
void myTMR0ISR(void) {
    ...
    // keep dutyVal static to avoid delay
    static uint8_t dutyVal;
    
    // write pwm for sound to work
    ...
    switch(currState){
        case MIC_IDLE:
            ...
            else if(pullAudio == true){
                currState = SD_ACQUIRE;
            }
            break;
        ...
        case SD_ACQUIRE:
           ...
           else if((color == 'B') && (blueEmpty == false)){
               dutyVal = blueBuffer[i];
               i++;
               if(i == BLOCK_SIZE){
                   blueEmpty = true;
                   color = 'R';
